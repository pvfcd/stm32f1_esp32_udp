...
u8 key = 0;
  ...
  if(USART_GetITStatus(USART3,USART_IT_RXNE)!=Bit_RESET) //检查 USART 是否发生中断
  {
      ...
      key=USART_ReceiveData(USART3);
			...
			{
			USART3_RX_STA = 0;
			USART3_RX_BUF[USART3_RX_STA&0X3FFF]=key;
			}
			/*确保接收到关键数据时可以在缓存数组的第一位*/
			if (key != 0)
				{
				USART3_RX_BUF[USART3_RX_STA&0X3FFF]=key ; //将收到的数据放入数组
				USART3_RX_STA ++;
				if (USART3_RX_STA > 200)
					{
					USART3_RX_STA = 0;
					}
				}
